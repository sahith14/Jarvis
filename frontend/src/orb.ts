...
export function createOrb(canvas: HTMLCanvasElement): Orb {
  ...
  function animate() {
    ...
    if (transitionEnergy > 0.05) {
      spinX += transitionEnergy * 0.012 * Math.sin(Date.now() * 0.0017);
      spinY += transitionEnergy * 0.015;
      spinZ += transitionEnergy * 0.008 * Math.cos(Date.now() * 0.0013);
    }
    ...
    let zTarget = Math.sin(Date.now() * 0.00012) * 8;
    if (state === "thinking") zTarget = Math.sin(Date.now() * 0.0003) * 15 + Math.sin(Date.now() * 0.0009) * 6;
    else if (state === "speaking") zTarget = Math.sin(Date.now() * 0.00015) * 6 - bass * 10;
    ...
    for (let i = 0; i < N; i++) {
      ...
      vel[i3] += Math.sin(Date.now() * 0.00005 + px) * 0.001 * currentSpeed;
      vel[i3+1] += Math.cos(Date.now() * 0.00006 + px * 1.3) * 0.001 * currentSpeed;
      vel[i3+2] += Math.sin(Date.now() * 0.000055 + px * 0.7) * 0.001 * currentSpeed;
      vel[i3] += Math.sin(Date.now() * 0.00002 + px * 2.1 + y * 0.1) * 0.0008 * currentSpeed;
      vel[i3+1] += Math.cos(Date.now() * 0.000025 + px * 1.7 + z * 0.1) * 0.0008 * currentSpeed;
      vel[i3+2] += Math.sin(Date.now() * 0.000022 + px * 0.9 + x * 0.1) * 0.0008 * currentSpeed;
      ...
      if (state === "speaking" && mid > 0.1) {
        const pulse = Math.sin(Date.now() * 0.008 + px);
    ...
    if (activeConnections.length > 0 && electronSpawnRate > 0.005) {
      if (activeElectrons.length < 3 && (Date.now() - lastElectronSpawn) > 1000) {
    ...
    camera.position.x = Math.sin(Date.now() * 0.00002) * 5;
    camera.position.y = Math.cos(Date.now() * 0.00003) * 3;
