...
try:
    ...
    class DummyPermissions:
        ...
        def confirm_action(self): return None
        def deny_action(self): return None
...
def load_memory():
    if MEMORY_FILE.exists():
        try:
            ...
            log.error(f"Error loading memory.json: {e}")
...
def _camera_watcher_loop():
    ...
    while True:
        try:
            ...
            log.warning(f"Camera watcher error: {e}")
            time.sleep(2)
...
def tool_play_youtube(query):
    ...
    url = f"https://www.youtube.com/watch?v={vid}&autoplay=1" if vid else f"https://www.youtube.com/results?search_query={urllib.parse.quote(str(query))}"
    webbrowser.open(url)
...
def handle_fast_command(text):
    ...
    # 1. Time
    if t in ("what time is it", "time", "what is the time"):
        return datetime.now().strftime("It's %I:%M %p")
        
    # 2. Strict YouTube
    if t == "open youtube":
        return dispatch_tool("play_youtube", {"query": ""})
    elif t.startswith("play "):
        query = t[5:].replace("on youtube", "").strip()
        return dispatch_tool("play_youtube", {"query": query})
        
    # 3. Strict Open App/Folder
    if t.startswith("open "):
        target = t[5:].strip()
        if target in ("downloads", "desktop", "documents"):
            return dispatch_tool("open_folder", {"path": target})
        elif target != "folder": # Ignore vague "open folder" to let LLM handle it
            return dispatch_tool("open_app", {"app_name": target})
    ...
    for _ in range(2):
        try:
            resp = await asyncio.to_thread(
                ...
                model=MODEL, messages=messages, tools=TOOLS, tool_choice="auto", temperature=0.6, max_tokens=512
            )
        except Exception as e:
            log.error(f"Groq API Error: {e}")
            return "I am currently having trouble reaching my language processing center."

        msg = resp.choices[0].message
        if resp.choices[0].finish_reason != "tool_calls" or not getattr(msg, "tool_calls", None):
...
async def voice_websocket(websocket: WebSocket):
        ...
        while True:
            try:
                if not awareness_queue.empty():
                    ...
                    await websocket.send_json({"type": "audio", "text": str(msg)})
                    await websocket.send_json({"type": "status", "state": "idle"})
                await asyncio.sleep(0.5)
            except Exception as e:
                log.error(f"Awareness Worker error: {e}")
                break
    ...
    try:
        while True:
            ...
            if not isinstance(msg, dict) or msg.get("type") != "transcript" or not msg.get("isFinal"):
                continue
            ...
            if permissions.has_pending():
                ...
                if tl in ("yes", "yeah", "yep", "do it", "proceed"):
                    ...
                    if action:
                        execute_fn = getattr(action, "execute_fn", None)
                        if execute_fn:
                            execute_args = getattr(action, "execute_args", {})
                            resp = execute_fn(**execute_args)
                        else:
                            command_text = getattr(action, "command_text", user_text)
                            resp = handle_fast_command(command_text) or await run_agent(command_text)
                    else:
                        resp = "No pending action found."
                    await websocket.send_json({"type": "audio", "text": str(resp)})
                    await websocket.send_json({"type": "status", "state": "idle"})
                    continue
